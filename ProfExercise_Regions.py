...
def main():   
    ...
    if len(sys.argv) <= 1:
        ...
        while key == -1:
            ...
            chosen_T, thresh_image = cv2.threshold(grayscale, 0, 255, cv2.THRESH_OTSU)

            numConnect, labelImage = cv2.connectedComponents(
                thresh_image, connectivity=8,
                ltype=cv2.CV_32S
            )
            
            print("Number of regions:", numConnect)
            color_regions = np.zeros(image.shape)
            for row in range(labelImage.shape[0]):
                for col in range(labelImage.shape[1]):
                    region = labelImage[row,col]
                    if region > 0:
                        region -= 1
                        index = region % len(colors)
                        color_regions[row,col] = colors[index]
            
            # Show the image
            cv2.imshow(windowName, image)
            cv2.imshow("THRESH", thresh_image)
            cv2.imshow("REGIONS", color_regions)
