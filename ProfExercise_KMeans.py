...
def main():   
    ...
    if len(sys.argv) <= 1:
        ...
        while key == -1:
            ...
            datapts = np.reshape(image, (-1,3)).astype("float32")
            _, bestLabels, centers = cv2.kmeans(
                datapts, K=10, bestLabels=None, 
            ...
            results = centers[bestLabels.flatten()]
            results = np.reshape(results, image.shape)
            
            # Show the image
            cv2.imshow(windowName, image)
            cv2.imshow("KMEANS", results)
