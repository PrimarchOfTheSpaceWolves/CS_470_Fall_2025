...
def main():   
    ...
    if len(sys.argv) <= 1:
        ...
        while key == -1:
            ...
            region_cnt = np.max(segments) + 1
            print("Number of regions:", region_cnt)
            
            for i in range(segments.shape[0]):
                for j in range(segments.shape[1]):
                    index = segments[i,j] % len(colors)
                    region_image[i,j] = colors[index]

            bound_image = mark_boundaries(image, segments)
            # Show the image
            cv2.imshow(windowName, image)
            cv2.imshow("SLIC", bound_image)
            cv2.imshow("REGIONS", region_image)
